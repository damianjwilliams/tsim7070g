...
bool newData = false;
bool setupDone = false; // Flag to check if setup has been done
...
// Core 0 task (reads serial input)
...
/*
  ...
  */
  ...
  String raw_data = SerialAT.readString();
  
  //Serial.println("raw input");
  //Serial.print(inputData);

  if(raw_data.indexOf("+CMT:") > 0)

      {

      parseSMS(raw_data);
   inputData = currentMessage;
   minX = -12 * inputData.length(); 
   //Serial.print(inputData);
      newData = true;
  modem.sendAT("+cmgd=,4");
      modem.waitResponse(1000L);
  ...
  //inputData = Serial.readStringUntil('\n'); // Read until newline character
      ...
      // Set flag to notify core 1 of new data
     
    

    // Add delay or other non-blocking tasks
    delay(10);
...
// Core 1 task (prints serial output)
...
while (1) {
   ...
   //Check if there is new data from core 0
...
void setup() {
  ...
  // 12 = 6 pixels/character * text size 2

  // Create tasks for each core
  xTaskCreatePinnedToCore(
    core0Task,   // Core 0 task function
    "Core0Task", // Task name
    10000,       // Stack size
    NULL,        // Parameter
    1,           // Priority
  ...
  xTaskCreatePinnedToCore(
    core1Task,   // Core 1 task function
    "Core1Task", // Task name
    10000,       // Stack size
    NULL,        // Parameter
    1,           // Priority
...
char delimiter = ',';
//String z_str =  parse_SMS_by_single_delim(for_mess, delimiter,0);
//String q_str =  parse_SMS_by_single_delim(for_mess, delimiter,1);
String date_str =  parse_SMS_by_delim(for_mess, delimiter,4);
String time_str =  parse_SMS_by_delim(for_mess, delimiter,5);
String message_str =  parse_SMS_by_delim(for_mess, delimiter,6);
...
if (message_str.indexOf("tony") >= 0) {
  Serial.println("_____________________-------________");
}
